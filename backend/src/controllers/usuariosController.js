...
async function listar(req, res, next) {
  try {
    const result = await db.query(
      'SELECT id, nome, telefone, perfil, ativo, created_at FROM usuarios ORDER BY created_at DESC'
    )
...
async function criar(req, res, next) {
  try {
    ...
    if (existente.rows.length) return res.status(409).json({ error: 'Telefone já cadastrado' })
    const senhaHash = await bcrypt.hash(senha, 10)
    const result = await db.query(
      'INSERT INTO usuarios (nome, telefone, senha, perfil) VALUES ($1, $2, $3, $4) RETURNING id, nome, telefone, perfil, ativo, created_at',
...
async function atualizar(req, res, next) {
  try {
    const { nome, telefone, perfil, ativo } = req.body
    const result = await db.query(
      'UPDATE usuarios SET nome = $1, telefone = $2, perfil = $3, ativo = $4 WHERE id = $5 RETURNING id, nome, telefone, perfil, ativo',
      [nome, telefone, perfil, ativo, req.params.id]
    )
